# Agent Guide — ep_disable_drag_and_drop
...
`ep_disable_drag_and_drop` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_disable_drag_and_drop`:
...
* Client: `aceInitInnerdocbodyHead`
